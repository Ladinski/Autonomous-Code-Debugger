...
def record_step(
    ...
    if action.action == "finish" and observation["success"]:
        assert action.finish is not None

        updated["final_diagnosis"] = action.finish.diagnosis
        updated["completion_status"] = "diagnosed"

    return updated


def _extract_arguments(action: AgentAction) -> dict:
    selected_args = getattr(action, action.action)
